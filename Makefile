...
ORG:="nalum"
REPO:="trust-manager-module"
TRUST_MANAGER_VERSION:=0.8.0
MV:=1.13.2
KV:=1.29.0
NAME:="trust-manager"
...
.PHONY: import-crds
import-crds: ## Update TRUST-Manager API CUE definitions
	@cd ./templates
	@curl -OLJ https://raw.githubusercontent.com/cert-manager/trust-manager/v$(TRUST_MANAGER_VERSION)/deploy/crds/trust.cert-manager.io_bundles.yaml
	@yq --yaml-output 'del(.metadata.labels["app.kubernetes.io/name"], .metadata.labels["app.kubernetes.io/instance"], .metadata.labels["app.kubernetes.io/version"])' trust.cert-manager.io_bundles.yaml > crds.yaml
	@cue import -f -o crds.cue -l 'strings.ToLower(kind)' -l 'metadata.name' -p templates crds.yaml
	@rm *crds.yaml *_bundles.yaml
